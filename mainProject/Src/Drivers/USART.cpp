...
char paramsFormatDefault[3] = {'s','u','\0'};
USART::USART(USART_TypeDef *instance):TX_pin(GPIOA),RX_pin(GPIOA){
    ...
    *settings = defaultSettings;
	...
	paramsPtr = (void*)parameters;
	paramsFormat = paramsFormatDefault;
}
...
void USART::gpioConfig()
{
    if(instance == USART1)
    {
        if(!gpioNameSpace::GPIO::isResourceAllocated(GPIOB,gpioNameSpace::GPIO_PIN_6) && \
        ...
        else if(!gpioNameSpace::GPIO::isResourceAllocated(GPIOA,gpioNameSpace::GPIO_PIN_9) && \
...
void USART::txItCallback(void){
    ...
    if(txBuffer[txCount] != '\0' && txComplete == false )
    {
...
void USART::parseUserCommands(void *paramsPtr, char *paramsFormat)
{
    ...
    while(*paramsFormat != '\0')
    {
        ...
        {
            char *asf;
            sscanf(bufferAddress,"%s", (char*)paramsAddress);
            asf = (char*)paramsAddress;
            paramsAddress = (char*)paramsAddress + ((64*sizeof(char)));
            bufferAddress += strlen(asf)*sizeof(char) + 1;
        }
        else if(*paramsFormat == 'u')
        {
            sscanf(bufferAddress,"%u", (unsigned int*)paramsAddress);
            paramsAddress = (unsigned int*)paramsAddress + ((sizeof(unsigned int)));
